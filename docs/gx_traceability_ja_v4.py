...
# -*- coding: utf-8 -*-
"""
GeneXus Traceability Builder v4
...
class DesignObject:
    ...
    feature_names: str = ""
    source_code: str = ""          # v4新增: Source标签内容
    source_events: str = ""        # v4新增: Events/Event内容
    source_rules: str = ""         # v4新增: Rules内容
    source_variables: str = ""     # v4新增: Variables摘要
...
def extract_source_from_element(elem: ET.Element) -> Dict[str, str]:
    """从XML元素中提取各种Source相关内容"""
    result = {
        "source_code": "",
        "source_events": "",
        "source_rules": "",
    ...
    def get_text_content(el: Optional[ET.Element]) -> str:
        """获取元素的完整文本内容（包括CDATA）"""
        if el is None:
            return ""
        # 尝试获取直接文本
    ...
    # 1. 提取 Source / SourceCode / Procedure Source
    source_tags = [
        "Source", "SourceCode", "ProcedureSource", "Code",
        "source", "sourceCode", "procedureSource", "code"
    ]
    for tag in source_tags:
        # 直接子元素
        source_el = elem.find(tag)
        if source_el is not None:
            content = get_text_content(source_el) or get_all_text(source_el)
            if content:
                result["source_code"] = content[:50000]  # 限制长度
                break
        # 在任意深度查找
        source_el = elem.find(f".//{tag}")
        if source_el is not None:
            content = get_text_content(source_el) or get_all_text(source_el)
            if content:
                result["source_code"] = content[:50000]
                break

    # 2. 提取 Events（Web Panel, Transaction 等的事件）
    events_content = []
    for events_container in elem.findall(".//Events") + elem.findall(".//events"):
        for event in events_container.findall("Event") + events_container.findall("event"):
            event_name = event.get("name") or event.get("Name") or event.findtext("Name") or event.findtext("name") or "UnnamedEvent"
            event_code = get_text_content(event.find("Source")) or get_text_content(event.find("source")) or get_text_content(event.find("Code")) or get_all_text(event)
            if event_code:
                events_content.append(f"[{event_name}]\n{event_code}")
    
    # 也查找直接的 Event 元素
    for event in elem.findall(".//Event") + elem.findall(".//event"):
        if event.getparent() is not None and event.getparent().tag in ("Events", "events"):
            continue  # 已经处理过
        event_name = event.get("name") or event.get("Name") or "Event"
        event_code = get_all_text(event)
        if event_code and event_code not in str(events_content):
            events_content.append(f"[{event_name}]\n{event_code}")
    
    if events_content:
        result["source_events"] = "\n---\n".join(events_content)[:50000]

    # 3. 提取 Rules（业务规则）
    rules_content = []
    for rules_container in elem.findall(".//Rules") + elem.findall(".//rules"):
        rules_text = get_all_text(rules_container)
        if rules_text:
            rules_content.append(rules_text)
    
    # 单独的 Rule 元素
    for rule in elem.findall(".//Rule") + elem.findall(".//rule"):
        rule_text = get_all_text(rule)
        if rule_text and rule_text not in str(rules_content):
            rules_content.append(rule_text)
    
    if rules_content:
        result["source_rules"] = "\n".join(rules_content)[:30000]

    # 4. 提取 Variables 摘要
    variables = []
    for var_container in elem.findall(".//Variables") + elem.findall(".//variables"):
        for var in var_container.findall("Variable") + var_container.findall("variable"):
            var_name = var.get("name") or var.get("Name") or var.findtext("Name") or var.findtext("name") or ""
            var_type = var.get("type") or var.get("Type") or var.findtext("Type") or var.findtext("type") or ""
            if var_name:
                variables.append(f"{var_name}:{var_type}" if var_type else var_name)
    
    if variables:
        result["source_variables"] = ", ".join(variables[:100])  # 最多100个变量

    return result


def extract_source_by_regex(xml_text: str) -> Dict[str, str]:
    """使用正则表达式提取Source内容（XML解析失败时的后备方案）"""
    result = {
        "source_code": "",
        "source_events": "",
        "source_rules": "",
    ...
    # 提取 <Source>...</Source> 或 <SourceCode>...</SourceCode>
    source_patterns = [
        r'<Source[^>]*>(.*?)</Source>',
        r'<SourceCode[^>]*>(.*?)</SourceCode>',
        r'<ProcedureSource[^>]*>(.*?)</ProcedureSource>',
        r'<Code[^>]*>(.*?)</Code>',
    ]
    ...
    # 提取 Events
    events_match = re.search(r'<Events[^>]*>(.*?)</Events>', xml_text, re.DOTALL | re.IGNORECASE)
    if events_match:
        events_text = events_match.group(1)
        # 提取各个 Event
        event_matches = re.findall(
            r'<Event[^>]*(?:name=["\']([^"\']*)["\'])?[^>]*>(.*?)</Event>',
            events_text, re.DOTALL | re.IGNORECASE
        )
        events_content = []
        for name, code in event_matches:
            name = name or "Event"
            code = re.sub(r'<!\[CDATA\[(.*?)\]\]>', r'\1', code, flags=re.DOTALL).strip()
            if code:
                events_content.append(f"[{name}]\n{code}")
        if events_content:
            result["source_events"] = "\n---\n".join(events_content)[:50000]
    
    # 提取 Rules
    rules_match = re.search(r'<Rules[^>]*>(.*?)</Rules>', xml_text, re.DOTALL | re.IGNORECASE)
    if rules_match:
        rules_text = rules_match.group(1)
        rules_text = re.sub(r'<!\[CDATA\[(.*?)\]\]>', r'\1', rules_text, flags=re.DOTALL)
        rules_text = re.sub(r'<[^>]+>', ' ', rules_text)  # 移除XML标签
        result["source_rules"] = rules_text.strip()[:30000]
    
    # 提取 Variables
    var_matches = re.findall(
        r'<Variable[^>]*name=["\']([^"\']*)["\'][^>]*(?:type=["\']([^"\']*)["\'])?',
        xml_text, re.IGNORECASE
    )
    if var_matches:
        variables = [f"{n}:{t}" if t else n for n, t in var_matches[:100]]
        result["source_variables"] = ", ".join(variables)
    
    return result


def extract_single_object_from_element(elem: ET.Element, xml_text: str = "") -> Tuple[Optional[str], Optional[str], str, str, str, Dict[str, str], Dict[str, str]]:
    """从单个XML元素中提取对象信息（v4: 增加source提取）"""
    props: Dict[str, str] = {}
    ...
    # v4新增: 提取Source相关内容
    source_info = extract_source_from_element(elem)

    return name, otype, title, desc, hints, props, source_info


def extract_all_objects_from_xml(xml_text: str) -> List[Tuple[Optional[str], Optional[str], str, str, str, Dict[str, str], Dict[str, str]]]:
    """从XML中提取所有Object元素的信息"""
...
def extract_objects_by_regex(xml_text: str) -> List[Tuple[Optional[str], Optional[str], str, str, str, Dict[str, str], Dict[str, str]]]:
    ...
    for match in object_pattern.finditer(xml_text):
        ...
        # v4: 提取Source
        source_info = extract_source_by_regex(inner_text) if inner_text else {
            "source_code": "", "source_events": "", "source_rules": "", "source_variables": ""
        }

        if name:
            results.append((name, otype, title, desc, "", props, source_info))

    return results


def extract_name_type_title_from_xml(xml_text: str) -> Tuple[Optional[str], Optional[str], str, str, str, Dict[str, str], Dict[str, str]]:
    """保持向后兼容的单对象提取函数"""
    results = extract_all_objects_from_xml(xml_text)
    if results:
        return results[0]
    return None, None, "", "", "", {}, {"source_code": "", "source_events": "", "source_rules": "", "source_variables": ""}
...
def create_design_object_from_extracted(
    ...
    return DesignObject(
        ...
        source_events=source_info.get("source_events", ""),
        source_rules=source_info.get("source_rules", ""),
        source_variables=source_info.get("source_variables", ""),
    )
...
def parse_design_file(path: Path, source_label: str) -> List[DesignObject]:
    ...
    if filename.endswith((".xml", ".gxobj", ".gxd", ".gxl")) or text.lstrip().startswith("<"):
        ...
        if extracted_objects:
            for name, otype, title, desc, hints, props, source_info in extracted_objects:
                obj = create_design_object_from_extracted(
                    name, otype, title, desc, hints, props, source_info,
        ...
        if not results:
            ...
            if is_probable_name(stem):
                ...
                # 尝试用正则提取Source
                source_info = extract_source_by_regex(text)
            ...
            if isinstance(data, list):
                for item in data:
                    if isinstance(item, dict):
                        ...
                        source_code = item.get("Source") or item.get("SourceCode") or ""
...
def build_design_objects(design_dir: Path) -> List[DesignObject]:
    ...
    for o in objs:
        ...
        if key not in best:
            ...
            cur = best[key]
            # v4: 也考虑source内容的丰富程度
            score_cur = len(cur.title) + len(cur.description) + len(cur.raw_hints) + len(cur.source_code) + len(cur.source_events)
            score_new = len(o.title) + len(o.description) + len(o.raw_hints) + len(o.source_code) + len(o.source_events)
            if score_new > score_cur:
                best[key] = o
    return list(best.values())


def build_feature_design_rows(objs: List[DesignObject], include_source: bool = True) -> List[Dict]:
    rows: List[Dict] = []
    for o in objs:
        ...
        for f in feats:
            ...
            if include_source:
                # 限制Source内容长度以避免Excel单元格过大
                row["ソースコード"] = (o.source_code[:10000] + "..." if len(o.source_code) > 10000 else o.source_code)
                row["イベント"] = (o.source_events[:5000] + "..." if len(o.source_events) > 5000 else o.source_events)
                row["ルール"] = (o.source_rules[:3000] + "..." if len(o.source_rules) > 3000 else o.source_rules)
                row["変数一覧"] = o.source_variables
            rows.append(row)
    return rows
...
def build_source_detail_rows(objs: List[DesignObject]) -> List[Dict]:
    """v4新增: 构建Source详情表（单独的Sheet）"""
    rows: List[Dict] = []
    for o in objs:
        if o.source_code or o.source_events or o.source_rules:
            rows.append({
                "オブジェクト名": o.object_name,
                "オブジェクト種別": o.object_type,
                "タイトル": o.title,
                "ソースコード": o.source_code,
                "イベントコード": o.source_events,
                "ルール": o.source_rules,
                "変数": o.source_variables,
                "元ファイル": o.source_file,
            })
    return rows
...
def autosize_worksheet(ws, max_width: int = 60) -> None:
    ...
    for row in ws.iter_rows(values_only=True):
        for i, val in enumerate(row, start=1):
            ...
            # 对于Source列，只计算第一行的宽度
            widths[i] = max(widths.get(i, 0), min(len(s), 100))
...
def save_excel(out_path: Path,
               ...
               source_detail_rows: List[Dict] = None) -> None:
    wb = Workbook()
    wb.remove(wb.active)
    ...
    if source_detail_rows:
        write_sheet(wb, "Source_Detail", source_detail_rows)
    wb.save(out_path)


def main():
    ap = argparse.ArgumentParser(description="GeneXus: Feature-Design & Design-Java traceability tables (v4 with Source extraction)")
    ...
    ap.add_argument("--no-source", action="store_true", help="Do not include source code in Feature_Design sheet")
    ap.add_argument("--source-sheet", action="store_true", help="Create separate Source_Detail sheet")
    args = ap.parse_args()
    ...
    # 统计有Source的对象数量
    source_count = sum(1 for o in objs if o.source_code or o.source_events or o.source_rules)
    print(f"[INFO] Objects with source code: {source_count}")
    ...
    include_source_in_main = not args.no_source
    feature_design_rows = build_feature_design_rows(objs, include_source=include_source_in_main)
    feature_group_rows = build_feature_group_rows(feature_design_rows)
    
    source_detail_rows = None
    if args.source_sheet:
        source_detail_rows = build_source_detail_rows(objs)
        print(f"[INFO] Source detail rows: {len(source_detail_rows)}")
    ...
    save_excel(out_path, feature_design_rows, feature_group_rows, design_java_rows, source_detail_rows)
