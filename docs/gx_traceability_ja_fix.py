...
def is_probable_name(s: str) -> bool:
    """GeneXusのオブジェクト名っぽい文字列かを判定（誤検出を避ける）"""
    if not s:
        return False
    s = s.strip()
    if len(s) > 80:
        return False
    # よくある誤検出（XML内の汎用タグ値など）を除外
    if s.lower() in {"name", "object", "type", "title", "caption", "description", "desc", "text", "value", "null"}:
        return False
...
def extract_name_type_title_from_xml(xml_text: str) -> Tuple[Optional[str], Optional[str], str, str, str]:
    """XML/擬似XMLから DesignObject の name/type/title/description をできるだけ拾う（属性形式も対応）"""
    def _clean(s: str) -> str:
        return re.sub(r"\s+", " ", s).strip()

    def _best_name(cands: List[str]) -> Optional[str]:
        uniq: List[str] = []
        seen = set()
        for c in cands:
            c = _clean(c)
        ...
        if not uniq:
            return None
        # 長いほど具体的な名前である可能性が高い（同点なら大文字を含む方を優先）
        uniq.sort(key=lambda s: (len(s), sum(ch.isupper() for ch in s)), reverse=True)
        return uniq[0]

    def _best_type(cands: List[str]) -> Optional[str]:
        uniq: List[str] = []
        seen = set()
        for c in cands:
            c = _clean(c)
            if not c or c in seen:
                continue
            seen.add(c)
            # type は英字中心ならOK（Transaction/WebPanel/Procedure など）
            if len(c) <= 80 and re.fullmatch(r"[A-Za-z][A-Za-z0-9_ ]*", c):
                uniq.append(c)
        if not uniq:
            return None
    ...
    # 名前候補（タグ内 / 属性 / 代表的なコンテナタグ）
    name_cands: List[str] = []
    # まずは明示的な ObjectName/ObjName を優先
    name_cands += re.findall(r"<\s*(?:ObjectName|ObjName)\s*>\s*([^<\s][^<]{0,120}?)\s*</\s*(?:ObjectName|ObjName)\s*>", xml_text, flags=re.I)
    # Nameタグ（誤検出もあるので is_probable_name 側で弾く）
    name_cands += re.findall(r"<\s*Name\s*>\s*([^<\s][^<]{0,120}?)\s*</\s*Name\s*>", xml_text, flags=re.I)
    # 属性形式（Name="X" / ObjectName='X' など）
    name_cands += re.findall(r"\b(?:ObjectName|ObjName|Name)\s*=\s*\"([^\">]{1,80})\"", xml_text, flags=re.I)
    name_cands += re.findall(r"\b(?:ObjectName|ObjName|Name)\s*=\s*'([^'>]{1,80})'", xml_text, flags=re.I)
    # よくあるコンテナ: <Object ... name="X" ...> / <KBObject ...>
    name_cands += re.findall(r"<\s*(?:Object|KBObject|DesignObject)\b[^>]*\bname\s*=\s*\"([^\">]{1,80})\"", xml_text, flags=re.I)
    name_cands += re.findall(r"<\s*(?:Object|KBObject|DesignObject)\b[^>]*\bName\s*=\s*\"([^\">]{1,80})\"", xml_text)

    name = _best_name(name_cands)

    # タイプ候補（タグ内 / 属性）
    type_cands: List[str] = []
    type_cands += re.findall(r"<\s*(?:ObjectType|Type|Category|Class)\s*>\s*([^<]{1,80}?)\s*</\s*(?:ObjectType|Type|Category|Class)\s*>", xml_text, flags=re.I)
    type_cands += re.findall(r"\b(?:ObjectType|Type|Category|Class)\s*=\s*\"([^\">]{1,80})\"", xml_text, flags=re.I)
    type_cands += re.findall(r"\b(?:ObjectType|Type|Category|Class)\s*=\s*'([^'>]{1,80})'", xml_text, flags=re.I)
    # <Object ... type="Transaction"> など
    type_cands += re.findall(r"<\s*(?:Object|KBObject|DesignObject)\b[^>]*\btype\s*=\s*\"([^\">]{1,80})\"", xml_text, flags=re.I)
    type_cands += re.findall(r"<\s*(?:Object|KBObject|DesignObject)\b[^>]*\bType\s*=\s*\"([^\">]{1,80})\"", xml_text)
    otype = _best_type(type_cands)

    # タイトル/説明（短いテキストを拾う）
    title = ""
    m = re.search(r"<\s*(?:Title|Caption|FormCaption|Text)\s*>\s*([^<]{1,200}?)\s*</\s*(?:Title|Caption|FormCaption|Text)\s*>", xml_text, flags=re.I)
    if m:
        title = _clean(m.group(1))

    desc = ""
    m = re.search(r"<\s*(?:Description|Desc|Documentation|Comment)\s*>\s*(.{1,400}?)\s*</\s*(?:Description|Desc|Documentation|Comment)\s*>", xml_text, flags=re.I | re.S)
    if m:
        desc = _clean(m.group(1))

    keywords = re.findall(r"\b(Search|Filter|Grid|For\s+each|Call|Export|Import|Approve|Login|Sync|REST|SDT|DataProvider)\b",
                          xml_text, flags=re.I)
...
def parse_design_file(path: Path, source_label: str) -> List[DesignObject]:
    ...
    if filename.endswith((".xml", ".gxobj", ".gxd", ".gxl")) or text.lstrip().startswith("<"):
        ...
        # XML内から名前が取れない/誤検出の場合、ファイル名から推定（XPZ展開物で多い）
        if not name or not is_probable_name(name):
            candidates = []
            candidates += re.findall(r"[A-Za-z_][A-Za-z0-9_]{1,80}", path.stem)
            candidates += re.findall(r"[A-Za-z_][A-Za-z0-9_]{1,80}", source_label)
            if candidates:
                candidates = [c for c in candidates if is_probable_name(c)]
                candidates.sort(key=len, reverse=True)
                name = candidates[0] if candidates else name
