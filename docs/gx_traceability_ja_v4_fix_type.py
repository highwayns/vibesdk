...
TYPE_KEYWORDS = {
    # より具体的なものを先に（同点のときにWPへ吸い込まれないようにする）
    "WWP": ["workwithplus", "workwith", "work with", "wwp"],
    ...
    "REST": ["expose as web service", "web service", "webservice", "rest"],
    "API": ["api object", "apiobject"],
    "Theme": ["theme"],
    # WP は最後。短い "wp" は誤検出が多いので使わない
    "WP": ["web panel", "webpanel", "web panel object", "webpanel object"],
}
...
def guess_type_from_text(text: str, filename: str, object_name: str = "") -> str:
    """テキスト/ファイル名/オブジェクト名から種別を推定する（誤検出対策版）。

    NOTE:
      - 2〜3文字の略語（dp, trn, sdt, proc, wwp 等）は単純な部分一致だと誤検出が多い。
        そこで「区切り（非英数）/末尾/数字」の境界でのみマッチさせる。
      - 同点の場合は「より具体的（優先度が高い）種別」を採用する。
    """
    t = (text[:20000] + "\n" + str(filename) + "\n" + (object_name or "")).lower()
    ...
    def kw_match(kw: str) -> bool:
        ...
        if " " in kw:
            return kw in t
        if len(kw) <= 3:
            pat = re.compile(r"(^|[^a-z0-9])" + re.escape(kw) + r"(?=([^a-z0-9]|$|[0-9]))")
    ...
    for k, kws in TYPE_KEYWORDS.items():
        ...
        if score == 0:
            continue
        if (score > best_score) or (score == best_score and spec > best_spec) or (
            score == best_score and spec == best_spec and priority.get(k, 99) < priority.get(best_type, 99)
        ):
            best_type = k
            best_score = score
            best_spec = spec
...
def extract_single_object_from_element(elem: ET.Element, xml_text: str = "") -> Tuple[Optional[str], Optional[str], str, str, str, Dict[str, str]]:
    ...
    # Source 标签（若存在 <Source>...</Source>，优先取其文本；否则取属性/Properties里的 Source 字段）
    source = ""
    try:
        src_elem = elem.find(".//Source")
        if src_elem is not None:
            # 保留换行，避免把源码压成一行
            source = ET.tostring(src_elem, encoding='unicode', method='text')
    except Exception:
        source = ""
    if not source:
        source = clean(attrs.get("source", "") or attrs.get("Source", "") or props.get("Source", "") or props.get("source", "") or props.get("SOURCE", ""))
    # 控制输出长度，避免 Excel 过大
    if source and len(source) > 10000:
        source = source[:10000] + "…(truncated)"
    return name, otype, title, desc, hints, source, props


def extract_all_objects_from_xml(xml_text: str) -> List[Tuple[Optional[str], Optional[str], str, str, str, str, Dict[str, str]]]:
...
def extract_objects_by_regex(xml_text: str) -> List[Tuple[Optional[str], Optional[str], str, str, str, str, Dict[str, str]]]:
    ...
    for match in object_pattern.finditer(xml_text):
        ...
        # Source 标签（<Source>...</Source> 或属性/Properties里的 Source）
        source = ""
        msrc = re.search(r"<\s*Source[^>]*>(.*?)</\s*Source\s*>", full_text, flags=re.I | re.S)
        if msrc:
            source = re.sub(r"\r\n?", "\n", msrc.group(1)).strip()
        if not source:
            source = clean(attrs.get("source", "") or attrs.get("Source", "") or props.get("Source", "") or props.get("source", "") or props.get("SOURCE", ""))
        ...
        if name:
            results.append((name, otype, title, desc, "", source, props))
...
def normalize_object_type(
    ...
    優先順：
      1) raw_type（文字列で判別できる場合）
      2) object_name の命名規則（例：*_DP01, *WW, *SDT）
      3) guessed（guess_type_from_text）
      4) filename/本文からの簡易推定
    """
    rt = (raw_type or "").strip().lower()
    # 1) raw_type 文字列で判別できるケース
    ...
    # 2) object_name 命名規則（GeneXusで非常に強い手掛かり）
    n = (object_name or "")
    n = (n.split(".")[-1] if n else n).lower()

    if re.search(r"(^|[_\.])dp\d*$", n) or n.endswith("_dp") or n.endswith("dp"):
        return "DP"
    if n.endswith("sdt") or "_sdt" in n:
        return "SDT"
    if n.endswith("ww") or n.endswith("wwp") or "workwith" in n:
        return "WWP"
    if n.endswith("trn") or "_trn" in n or n.endswith("transaction"):
        return "Transaction"
    if n.endswith("proc") or n.endswith("procedure") or n.startswith("proc") or n.startswith("p_") or n.startswith("prc"):
        return "Procedure"
    if n.endswith("sd") or n.endswith("sdpanel") or "sdpanel" in n:
        return "SD"
    if n.endswith("api") or n.endswith("apiobject") or "apiobject" in n:
        return "API"
    if n.endswith("theme") or "theme" in n:
        return "Theme"
    if n.endswith("wp") or "webpanel" in n:
        return "WP"

    # 3) guessed（既に対策済みの推定値）
    ...
    if "proc" in fn or "procedure" in fn:
        return "Procedure"
    if re.search(r"(^|[_\.])dp\d*", fn) or "dataprovider" in fn:
        return "DP"
    if "sd" in fn and ("panel" in fn or "smart" in text.lower()):
        return "SD"
    if "webpanel" in fn or "web panel" in text.lower():
        return "WP"
    return "Unknown"
...
def create_design_object_from_extracted(
    ...
    guessed = guess_type_from_text(text, filename, name)
    object_type = normalize_object_type(otype, guessed, name, filename, text)
...
def parse_design_file(path: Path, source_label: str) -> List[DesignObject]:
    ...
    if filename.endswith((".xml", ".gxobj", ".gxd", ".gxl")) or text.lstrip().startswith("<"):
        ...
        if extracted_objects:
            for name, otype, title, desc, hints, source, props in extracted_objects:
                obj = create_design_object_from_extracted(
                    name, otype, title, desc, hints, source, props,
        ...
        if not results:
            ...
            if is_probable_name(stem):
                guessed = guess_type_from_text(text, path.name, stem)
                object_type = normalize_object_type(None, guessed, stem, path.name, text)
                ...
                for item in data:
                    if isinstance(item, dict):
                        ...
                        if name and is_probable_name(name):
                            guessed = guess_type_from_text(str(item), path.name, name)
                            object_type = normalize_object_type(otype, guessed, name, path.name, str(item))
    ...
    guessed = guess_type_from_text(text, path.name, name or '')
    object_type = normalize_object_type(otype, guessed, name or '', path.name, text)
